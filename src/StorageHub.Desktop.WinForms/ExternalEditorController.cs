...
internal sealed class ExternalEditorController : IAsyncDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (decision.DontShowAgain)
        {
            ...
            {
                Show(owner, Ui.Validation.StorageHubCouldNotSaveThatPreferenceSo, MessageBoxIcon.Information);
            }
        }
    ...
    private static void Show(IWin32Window owner, string message, MessageBoxIcon icon) =>
        _ = MessageBox.Show(owner, message, Ui.Dialogs.ExternalEditorCaption, MessageBoxButtons.OK, icon);
    ...
    private sealed class ExternalEditSession : IDisposable
    {
        ...
        internal void Start(string? editorPath)
        {
            ...
            _ = Process.Start(start) ?? throw new InvalidOperationException(Ui.Validation.WindowsCouldNotStartTheConfiguredEditor);
        }
        ...
        private async void DebounceTick(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    StopForUnsafeFile(Ui.Validation.TheEditorReplacedTheTemporaryFileWith);
                    return;
                }
                ...
                if (file.Length > _maximumBytes)
                {
                    _ = MessageBox.Show(
                        _owner,
                        Ui.Format(Ui.Dialogs.ExternalEditorTooLargeFormat, _maximumBytes / 1024),
                ...
                var choice = MessageBox.Show(
                    _owner,
                    Ui.Format(Ui.Dialogs.UploadEditedFilePromptFormat, Path.GetFileName(_localPath)),
                    Ui.Dialogs.UploadEditedFileCaption,
                ...
                InvalidOperationException or TimeoutException or System.Text.Json.JsonException)
            {
                _ = MessageBox.Show(
                    _owner,
                    Ui.Format(Ui.Dialogs.ExternalEditorProcessFailedFormat, error.Message),
                    Ui.Dialogs.ExternalEditorCaption,
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
            }
