...
public sealed class WorkspaceControl : UserControl
{
        ...
        IReadOnlyDictionary<Guid, BrowserPaneState>? initialStates = null)
    {
        ...
        Dock = DockStyle.Fill;
        AccessibleName = Ui.Format(Ui.Shell.WorkspaceAccessibleNameFormat, name);
        AccessibleDescription = Ui.Shell.WorkspaceAccessibleDescription;
        ...
        var hint = new ToolStripLabel(Ui.Shell.DragPaneHeadersHint)
        {
        ...
        var paste = new ToolStripButton(Ui.Shell.PasteToActivePane)
        {
        ...
        var clear = new ToolStripButton(Ui.Shell.ClipboardClear)
        {
    ...
    public string WorkspaceName
    {
        ...
        set
        {
            ...
            // Set through the property means somebody chose this name, so saving under a different
            // filename later leaves it alone.
            HasExplicitName = true;
            AccessibleName = Ui.Format(Ui.Shell.WorkspaceAccessibleNameFormat, value);
            MarkDirty();
        }
    }
    ...
    /// <summary>
    /// True once the workspace has been given a name deliberately -- through the rename dialog, or
    /// by being loaded from a file that carried one. A workspace still on its generated default
    /// ("Workspace 1") has not.
    /// </summary>
    [System.ComponentModel.DesignerSerializationVisibility(
        System.ComponentModel.DesignerSerializationVisibility.Hidden)]
    public bool HasExplicitName { get; private set; }

    /// <summary>
    /// Marks the current name as a deliberate one, for a workspace loaded from a file that already
    /// carried it. Without this every opened workspace would look renameable-by-filename.
    /// </summary>
    public void MarkNameAsExplicit() => HasExplicitName = true;

    public void Save(string path)
    {
        // Choosing a filename names the workspace, unless it already has a name somebody picked.
        // Done before the capture, so the new name is what lands in the file and the workspace is
        // not left dirty by its own rename.
        var candidate = Path.GetFileNameWithoutExtension(path);
        if (!HasExplicitName && !string.IsNullOrWhiteSpace(candidate))
        {
            _workspaceName = candidate.Trim();
            HasExplicitName = true;
            AccessibleName = Ui.Format(Ui.Shell.WorkspaceAccessibleNameFormat, _workspaceName);
        }
    ...
    private Control BuildNode(WorkspaceLayoutNode node)
    {
        ...
        // SplitterMoved does not know a drag from a programmatic move: the control raises it from
        // its own layout on every resize, and again when the ratio above is restored. Every window
        // resize therefore re-derived the ratio from the rounded pixel distance and marked the
        // workspace dirty, so a workspace that was only ever looked at prompted to be saved on
        // exit. SplitterMoving is raised only while the user is dragging (or nudging with the
        // keyboard), so it is the signal that the distance that follows is theirs.
        var userMoving = false;
        split.SplitterMoving += (_, _) => userMoving = true;
    ...
    private Panel BuildPaneFrame(Guid paneId)
    {
        ...
        var showHeader = new ToolStripMenuItem("Show Connection Bar") { CheckOnClick = true };
        showHeader.Click += (_, _) => _panes[paneId].HeaderVisible = showHeader.Checked;
        actions.DropDownItems.Add(showHeader);
        // The frame is rebuilt on every RebuildLayout, and HydrateAsync restores pane state after
        // the layout exists, so a check state captured at build time would show the pre-hydration
        // value. Read the pane when the menu opens instead.
        actions.DropDownOpening += (_, _) => showHeader.Checked = _panes[paneId].HeaderVisible;
    ...
    private void RenumberHeaders()
    {
        ...
        foreach (var header in Descendants<ToolStrip>(_layoutHost).Where(control => control.Tag is Guid))
        {
            ...
            title.Text = label;
            header.AccessibleName = Ui.Format(Ui.Pane.PaneHeaderAccessibleNameFormat, label);
            pane.AccessibleName = Ui.Format(Ui.Pane.PaneBrowserAccessibleNameFormat, label);
            if (id == _activePaneId)
            {
                title.Text += " (Active)";
            }
...
internal sealed class NewWorkspaceForm : Form
{
    ...
    {
        Text = Ui.Shell.NewWorkspaceCaption;
        AccessibleName = Ui.Shell.NewWorkspaceChooserAccessibleName;
        ...
        _remember = new StorageHubCheckBox
        {
