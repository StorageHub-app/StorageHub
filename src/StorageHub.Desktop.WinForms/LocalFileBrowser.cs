...
public sealed class LocalFileBrowserDataSource : IPagedLocalFileBrowserDataSource, IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        lock (_sessionGate)
        {
            ...
            {
                throw new InvalidOperationException(Ui.Validation.TheLocalListingContinuationIsNoLonger);
            }
            else
            {
    ...
    private static LocalBrowserEntry[] EnumerateDrives(CancellationToken cancellationToken)
    {
        ...
        foreach (var drive in DriveInfo.GetDrives().OrderBy(static drive => drive.Name, StringComparer.OrdinalIgnoreCase))
        {
            ...
            var status = Ui.Validation.Unavailable;
            try
            {
            ...
            {
                status = Ui.Validation.Unavailable;
            }
            ...
            entries.Add(new LocalBrowserEntry(
                ...
                Modified: null,
                type,
                isReady ? status : Ui.Validation.Unavailable));
        }
    ...
    private static LocalBrowserEntry CreateEntry(FileSystemInfo item)
    {
        ...
        try
        {
            ...
            {
                statusParts.Add(Ui.Validation.ReadOnly);
            }
        }
        catch (Exception error) when (LocalBrowserErrors.IsExpected(error))
        {
            statusParts.Clear();
            statusParts.Add(Ui.Validation.MetadataUnavailable);
        }
        ...
        return new LocalBrowserEntry(
            ...
            length,
            modified,
            isContainer ? Ui.Validation.FileFolder : LocalBrowserPresentation.DescribeFileType(item.Extension),
            string.Join(" · ", statusParts));
    }

    private static string DescribeDriveType(DriveType driveType) => driveType switch
    {
        DriveType.Fixed => Ui.Validation.LocalDisk,
        DriveType.Network => "Network",
        DriveType.Removable => Ui.Validation.Removable,
    ...
    private LocalBrowserLocation? ResolveTarget(
        ...
        LocalBrowserLocation? requestedLocation) => kind switch
        {
            LocalBrowserNavigationKind.Navigate => requestedLocation ??
                throw new ArgumentException(Ui.Validation.ADestinationIsRequiredForDirectNavigation, nameof(requestedLocation)),
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(kind), kind, Ui.Validation.UnknownNavigationKind)
        };
    ...
    private void CommitNavigation(LocalBrowserNavigationKind kind, LocalBrowserLocation target)
    {
        switch (kind)
        {
            ...
            case LocalBrowserNavigationKind.Refresh:
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(kind), kind, Ui.Validation.UnknownNavigationKind);
        }
    }
    ...
    public static string ToSafeMessage(Exception error)
    {
        ...
        return error switch
        {
            UnauthorizedAccessException or SecurityException =>
                Ui.Validation.AccessDeniedYouDoNotHavePermission,
            DirectoryNotFoundException or DriveNotFoundException =>
                Ui.Validation.ThisLocationCouldNotBeFoundIt,
            PathTooLongException =>
                Ui.Validation.ThisFolderPathIsTooLongFor,
            ArgumentException or NotSupportedException =>
                Ui.Validation.TheAddressIsNotAValidFolder,
            IOException =>
                Ui.Validation.WindowsCouldNotReadThisLocationIt,
            _ => Ui.Validation.StorageHubCouldNotOpenThisLocation
        };
    }
}
