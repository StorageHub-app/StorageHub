...
public interface IObjectInspectorAgentClient : IAsyncDisposable
{
    ...
    Task<EditableFileDownloadResponse> DownloadEditableFileAsync(
        ...
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisInspectorClientDoesNotSupportBounded);

    Task<EditableFileUploadResponse> UploadEditedFileAsync(
        EditableFileUploadRequest request,
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisInspectorClientDoesNotSupportBounded2);

    Task<StorageDirectoryEnsureResponse> EnsureDirectoryAsync(
        StorageDirectoryEnsureRequest request,
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisInspectorClientDoesNotSupportDirectory);

    Task<StorageDirectoryCreateResponse> CreateDirectoryAsync(
        StorageDirectoryCreateRequest request,
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisInspectorClientDoesNotSupportExplicit);

    Task<StorageFileCreateResponse> CreateFileAsync(
        StorageFileCreateRequest request,
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisInspectorClientDoesNotSupportEmpty);

    Task<StorageItemRenameResponse> RenameItemAsync(
        StorageItemRenameRequest request,
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisInspectorClientDoesNotSupportRenaming);

    Task<StorageItemDeleteResponse> DeleteItemAsync(
        StorageItemDeleteRequest request,
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisInspectorClientDoesNotSupportDeleting);
}
...
public sealed class NamedPipeObjectInspectorAgentClient : IObjectInspectorAgentClient
{
        ...
        where TResponse : class
    {
        ...
        catch (OperationCanceledException error) when (!cancellationToken.IsCancellationRequested)
        {
            throw new TimeoutException(
                Ui.Validation.TheLocalAgentInspectorRequestTimedOut,
                error);
        }
        ...
        try
        {
            ...
            catch (JsonException error)
            {
                throw new InvalidDataException(
                    Ui.Validation.TheLocalAgentReturnedInvalidObjectInspector,
                    error);
            }
        ...
        catch (OperationCanceledException error) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new TimeoutException(Ui.Validation.TheLocalAgentInspectorRequestTimedOut2, error);
        }
        ...
        string expectedMessageType)
    {
        ...
        {
            throw new InvalidDataException(Ui.Validation.TheLocalAgentRejectedTheObjectInspector);
        }
        ...
        where TRequest : class
    {
        if (request is null || !validBounds)
        {
            throw new ArgumentException(
                Ui.Validation.TheObjectInspectorRequestIsOutsideThe,
                parameterName);
        }
    }
    ...
    private static InvalidDataException InvalidResponse() =>
        new(Ui.Validation.TheLocalAgentReturnedObjectInspectorData);
    ...
    private static void ValidateOptions(ObjectInspectorAgentClientOptions options)
    {
        ...
        if (options.ConnectTimeout <= TimeSpan.Zero || options.ConnectTimeout > TimeSpan.FromSeconds(15))
        {
            throw new ArgumentOutOfRangeException(
                nameof(options),
                Ui.Validation.TheConnectTimeoutMustBeAtMost);
        }
        ...
        if (options.RequestTimeout <= TimeSpan.Zero || options.RequestTimeout > TimeSpan.FromMinutes(1))
        {
            throw new ArgumentOutOfRangeException(
                nameof(options),
                Ui.Validation.TheRequestTimeoutMustBeAtMost);
        }
    }
