...
internal sealed class SettingsExportService
{
    ...
    internal static void Write(string path, SettingsExportDocument document, string? password)
    {
        ...
        {
            throw new ArgumentException(Ui.Validation.TheExportPathMustBeAbsolute, nameof(path));
        }
        ...
        {
            throw new ArgumentException(Ui.Validation.TheseSettingsAreTooLargeToExport, nameof(document));
        }

        var full = Path.GetFullPath(path);
        var parent = Path.GetDirectoryName(full)
            ?? throw new IOException(Ui.Validation.TheExportDirectoryIsUnavailable);
    ...
    private static void RejectReparsePoint(string path)
    {
        ...
        {
            throw new IOException(Ui.Validation.RefusingToWriteTheExportThroughA);
        }
    }
}
