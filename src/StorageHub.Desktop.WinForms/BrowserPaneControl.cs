...
public sealed class BrowserPaneControl : UserControl
{
    ...
    /// <summary>
    /// A property rather than a const: the text follows the current language, and it is compared
    /// against the label already on screen to decide whether the notice needs replacing.
    /// </summary>
    private static string EmptyFolderMessage => Ui.Pane.FolderIsEmpty;
    ...
    /// <summary>
    /// The folder tree's own images, deliberately not the file list's.
    /// </summary>
    /// <remarks>
    /// The file list's ImageList grows as association icons are resolved, and adding to a live
    /// ImageList recreates its native handle, which blanks the icons of every control using it
    /// until each one is invalidated again. The tree shared that list while only ever needing
    /// "connection" and "folder", so it was collateral damage: its folder icons vanished as the
    /// pointer moved over the file list and reappeared when the tree happened to repaint.
    /// </remarks>
    ...
    private readonly StorageHubButton _connectionButton;
    ...
    private readonly StorageHubToolStripField _addressBox;
    private readonly StorageHubToolStripField _filterBox;
    ...
    private readonly TableLayoutPanel _paneHeader;
        ...
        Func<Guid, string, SshTerminalForm>? sshTerminalFactory = null)
    {
        ...
        AccessibleDescription = Ui.Format(Ui.Pane.BrowseEndpointFormat, title.ToLower(CultureInfo.CurrentCulture));
        ...
        // One row, not three columns over two text rows. The connection name used to appear both
        // here and on the picker button, and the Manage button duplicated the main toolbar, the
        // Welcome page, the connections panel and Settings -- 88px per pane, or 352px in a
        // four-pane workspace, to say the same thing four times. The height is derived from the
        // font so a DPI or font change still fits the text.
        ...
        {
            Text = showLocalDefault ? Ui.Pane.ThisPc : Ui.Pane.ConnectionsHome,
            Dock = DockStyle.Fill,
        ...
        // The name takes only the width it needs, so its badges sit beside it rather than being
        // pushed to the far edge of a fixed column with a gap in between. The third column soaks
        // up the slack and lets a long name ellipsize instead of shoving the badges off.
        ...
        [
            new ConnectionCardModel(Ui.Pane.ThisPc, StorageProviderKind.Local, Ui.Pane.LocalDrives, Ui.Pane.Ready, true),
            new ConnectionCardModel(Ui.Pane.ConnectionsHome, StorageProviderKind.Local, Ui.Pane.SavedConnections, "Browse")
        ]);
        ...
        _connectionButton = new StorageHubButton
        {
            Dock = DockStyle.Fill,
        ...
        {
            Text = showLocalDefault ? Ui.Pane.StateReady : Ui.Pane.StateChooseConnection,
            Dock = DockStyle.Fill,
            TextAlign = ContentAlignment.MiddleLeft,
        ...
        header.Layout += (_, _) =>
        {
            ...
            var identityWidth = veryCompact ? 160 : compact ? 260 : 380;
            // Each of these assignments schedules another layout of the header, and this runs
            // inside a layout, so writing them unconditionally laid the header out twice for
            // every pass and repainted the whole strip on every step of a window drag.
            if ((int)header.ColumnStyles[0].Width != identityWidth)
            {
                header.ColumnStyles[0].Width = identityWidth;
            }

            if (badges.Visible == veryCompact)
            {
        ...
        _forwardButton = CreateButton(UiGlyph.Forward, Ui.Pane.Forward);
        _upButton = CreateButton(UiGlyph.Up, "Up");
        _refreshButton = CreateButton(UiGlyph.Refresh, Ui.Pane.Refresh);
        _loadMoreButton = new ToolStripButton(Ui.Pane.LoadMore)
        {
            DisplayStyle = ToolStripItemDisplayStyle.Text,
            AccessibleName = Ui.Pane.LoadNextPageLabel,
            AccessibleDescription = Ui.Pane.LoadNextPageHint,
            ToolTipText = Ui.Pane.LoadNextPage,
        ...
        _addressBox = new StorageHubToolStripField
        {
            ...
            AccessibleDescription = showLocalDefault
                ? Ui.Pane.LocalAddressHint
                : Ui.Pane.RemoteAddressAccessibleName
        };
        ...
        _navigation.Items.Add(new ToolStripSeparator());
        _navigation.Items.Add(new ToolStripLabel(Ui.Pane.FilterLabel) { ForeColor = StorageHubTheme.TextMuted });
        _filterBox = new StorageHubToolStripField
        {
            ...
            AccessibleDescription = Ui.Pane.FilterAccessibleName,
            ToolTipText = Ui.Pane.FilterPlaceholder
        };
        ...
        _newFolderButton = CreateCommandButton(UiGlyph.Folder, Ui.Pane.NewFolder, Ui.Pane.CreateFolderHere);
        _copyButton = CreateCommandButton(UiGlyph.File, Ui.Pane.Copy, Ui.Pane.StageForCopying);
        _moveButton = CreateCommandButton(UiGlyph.Forward, Ui.Pane.Move, Ui.Pane.StageForMoving);
        _pasteButton = CreateCommandButton(UiGlyph.Save, Ui.Pane.Paste, Ui.Pane.ReviewAndPaste);
        _deleteButton = CreateCommandButton(UiGlyph.Delete, Ui.Pane.Delete, Ui.Pane.ReviewAndDelete);
        ...
        _directoryTree = new TreeView
        {
            ...
            AccessibleDescription = Ui.Pane.FoldersAndRoots
        };
        _browserImages = CreateBrowserImageList();
        _shellIcons = new WindowsShellIconProvider(_browserImages);
        _treeImages = CreateBrowserImageList();
        _directoryTree.ImageList = _treeImages;
        var rootNode = _directoryTree.Nodes.Add(showLocalDefault ? Ui.Pane.ThisPc : Ui.Pane.Connections);
        ...
        // Dark scrollbars and TVS_EX_DOUBLEBUFFER for the folder tree. The theme walk only
        // reaches panes that exist when the shell's handle is created; one opened later kept
        // light scrollbars and painted every expand and scroll straight to the screen.
        StorageHubTheme.ApplyNativeChrome(_directoryTree);
        ...
        var open = CreateContextMenuItem(Ui.Pane.Open, UiGlyph.Folder);
        var newFolder = CreateContextMenuItem(Ui.Pane.NewFolder, UiGlyph.Folder, UiCommandIds.EditNewFolder);
        var newFile = CreateContextMenuItem(Ui.Pane.NewEmptyFile, UiGlyph.File, UiCommandIds.EditNewEmptyFile);
        var edit = CreateContextMenuItem(Ui.Pane.EditInExternalEditor, UiGlyph.File);
        var rename = CreateContextMenuItem(Ui.Pane.Rename, UiGlyph.File, UiCommandIds.EditRename);
        var batchRename = CreateContextMenuItem(Ui.Pane.BatchRename, UiGlyph.File, UiCommandIds.EditBatchRename);
        var copy = CreateContextMenuItem(Ui.Pane.Copy, UiGlyph.File, UiCommandIds.EditCopy);
        var cut = CreateContextMenuItem(Ui.Pane.Cut, UiGlyph.Forward, UiCommandIds.EditCut);
        var paste = CreateContextMenuItem(Ui.Pane.Paste, UiGlyph.Save, UiCommandIds.EditPaste);
        var delete = CreateContextMenuItem(Ui.Pane.Delete, UiGlyph.Delete, UiCommandIds.EditDelete);
        var refresh = CreateContextMenuItem(Ui.Pane.Refresh, UiGlyph.Refresh, UiCommandIds.ViewRefresh);
        var selectAll = CreateContextMenuItem(Ui.Pane.SelectAll, UiGlyph.Test, UiCommandIds.EditSelectAll);
        var inspectObject = CreateContextMenuItem(Ui.Pane.Properties, UiGlyph.Info, UiCommandIds.EditProperties);
        ...
        _fileContextMenu.Opening += (_, _) =>
        {
            ...
            open.Text = selectedParent
                ? Ui.Pane.GoUpOneLevel
                : selectedContainer ? Ui.Pane.Open : Ui.Pane.OpenInExternalEditor;
        ...
        _summary = new Label
        {
            ...
            ForeColor = StorageHubTheme.TextMuted,
            BackColor = StorageHubTheme.Surface,
            AccessibleName = Ui.Format(Ui.Pane.ItemSummaryAccessibleNameFormat, title)
        };
        ...
        Controls.Add(_paneHeader);
        Controls.Add(_accentBar);

        if (showLocalDefault)
        {
            SetItems([new BrowserListItem(Ui.Pane.LoadingLocalDrives, string.Empty, Ui.Pane.ColumnStatus, string.Empty, string.Empty)]);
        }
        else
        {
            SetItems([new BrowserListItem(
                Ui.Pane.LoadingConnections,
                string.Empty,
                Ui.Pane.ColumnStatus,
                string.Empty,
                string.Empty)]);
        }
    ...
    /// <summary>
    /// Whether the one-line connection header is shown. Hiding it gives the whole pane to the
    /// listing, which is the point in a four-pane workspace; the choice is per pane and rides
    /// along in the saved workspace.
    /// </summary>
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public bool HeaderVisible
    {
        get => _paneHeader.Visible;
        set
        {
            if (_paneHeader.Visible == value)
            {
                return;
            }

            _paneHeader.Visible = value;
            RaiseStateChanged();
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _paneHeader.Visible = !state.HeaderHidden;
            UpdateSortColumnHeaders();
            if (state.ContentKind == PaneContentKind.ThisPc)
            {
                SelectChoice(Ui.Pane.ThisPc);
            ...
            {
                SelectChoice(Ui.Pane.ConnectionsHome);
            ...
            if (index < 0)
            {
                ...
                ShowError(Ui.Format(
                    Ui.Pane.SavedProfileUnavailableFormat,
                    state.DisplayNameHint ?? state.ProfileId.Value.ToString("D", CultureInfo.InvariantCulture)));
                return;
            }
            ...
            {
                ShowBrowserSurface();
                SetItems([new BrowserListItem(Ui.Pane.Disconnected, string.Empty, Ui.Pane.ColumnStatus, string.Empty, Ui.Pane.SelectProfileToConnect)]);
                _remoteReconnectPending = true;
                _errorBanner.Cursor = Cursors.Hand;
                ShowNotice(Ui.Pane.AutoReconnectDisabled);
                return;
            }
    ...
    public StorageResult<PaneSelectionSnapshot> CaptureSelectionSnapshot()
    {
        ...
        foreach (var index in _fileList.SelectedIndices.Cast<int>().Order())
        {
            ...
            {
                return SelectionSnapshotFailure(Ui.Pane.SelectionChangedWhileCapturing);
            }
    ...
    public StorageResult<TransferQueueAddress> CaptureShellImportDestination(string? relativePath = null)
    {
            ...
            context.Value.ConnectionId is not { } id || string.IsNullOrWhiteSpace(context.Value.RootIdentity))
        {
            return StorageResult<TransferQueueAddress>.Fail(new StorageFailure(
                ...
                Ui.Pane.DropRequiresConnection));
        }
    ...
    {
        string[] labels = [Ui.Pane.ColumnName, Ui.Pane.ColumnSize, "Type", Ui.Pane.ColumnModified, Ui.Pane.ColumnStatus];
        ...
        LocalBrowserLocation? location = null)
    {
        ...
        switch (result.Status)
        {
            ...
            case LocalBrowserNavigationStatus.Failed:
                ShowError(result.ErrorMessage ?? Ui.Pane.LocationCouldNotOpen);
            ...
            case LocalBrowserNavigationStatus.NoTarget:
                break;
            default:
                throw new InvalidOperationException(Ui.Pane.InvalidNavigationResult);
        }
    ...
    private async Task LoadMoreLocalAsync()
    {
        ...
        try
        {
            ...
            {
                ShowError(result.ErrorMessage ?? Ui.Pane.IndexingFailed);
            }
        }
        catch (ObjectDisposedException)
        {
    ...
    private async Task LoadRemoteConnectionsAsync(bool preserveCurrentSurface = false)
    {
        ...
        switch (result.Status)
        {
            ...
            case RemoteBrowserOperationStatus.Failed:
                ...
                {
                    PresentConnectionsHome(_remoteBrowser.Connections);
                    ShowError(result.ErrorMessage ?? Ui.Pane.ConnectionsUnavailable);
                }
            ...
            case RemoteBrowserOperationStatus.Superseded:
                break;
            default:
                throw new InvalidOperationException(Ui.Pane.InvalidConnectionLoadResult);
        }
    ...
    private async Task SelectRemoteConnectionAsync(Guid connectionId)
    {
        ...
        var sequence = ++_uiNavigationSequence;
        SetNavigationBusy(true);
        _connectionState.Text = Ui.Pane.StateTestingConnection;
        HideError();
    ...
    private void PresentRemoteResult(RemoteBrowserNavigationResult result)
    {
        switch (result.Status)
        {
            case RemoteBrowserOperationStatus.Succeeded when result.Snapshot is not null:
                ...
                break;
            case RemoteBrowserOperationStatus.Failed:
                ShowError(result.ErrorMessage ?? Ui.Pane.RemoteLocationCouldNotOpen);
                ...
                else
                {
                    _addressBox.Text = "/";
                    SetItems([new BrowserListItem(
                        Ui.Pane.ConnectionCouldNotOpen,
                        string.Empty,
                        Ui.Pane.ColumnStatus,
                        string.Empty,
                        string.Empty)]);
            ...
            case RemoteBrowserOperationStatus.NoTarget:
                break;
            default:
                throw new InvalidOperationException(Ui.Pane.InvalidRemoteNavigationResult);
        }
    ...
    private void PresentRemoteSnapshot(RemoteBrowserSnapshot snapshot, bool appendPage = false)
    {
        ...
        _connectionState.Text = Ui.Pane.StateReady;
        _connectionState.ForeColor = StorageHubTheme.Success;
        _connectionState.AccessibleDescription = Ui.Format(Ui.Pane.ShowingConnectionFormat, snapshot.Connection.DisplayName, snapshot.DisplayPath);
    ...
    private void ReplaceRemoteConnectionChoices(IReadOnlyList<ConnectionSummary> connections)
    {
        ...
        var previousWasThisPc = _localBrowsingEnabled &&
            string.Equals(previous?.Name, Ui.Pane.ThisPc, StringComparison.Ordinal);
        _updatingConnectionChoices = true;
        ...
        try
        {
            ...
            _connectionSelector.Items.Add(new ConnectionCardModel(
                ...
                StorageProviderKind.Local,
                Ui.Pane.SavedConnections,
                "Browse"));
            var savedCards = connections.Select(connection => new ConnectionCardModel(
                    connection.DisplayName,
                    MapProvider(connection.Provider),
                    connection.FolderPath ?? Ui.Pane.SavedConnection,
                    connection.IsEnabled ? DescribeConnectionHealth(connection.Health) : Ui.Pane.Disabled,
    ...
    {
        ShowBrowserSurface();
        _addressBox.Text = Ui.Pane.Connections;
        _directoryTree.Nodes.Clear();
        var connectionsRoot = _directoryTree.Nodes.Add(Ui.Pane.Connections);
        SetTreeNodeIcon(connectionsRoot, "connection");
        if (connections.Count == 0)
        {
            SetItems([new BrowserListItem(
                Ui.Pane.NoEnabledConnections,
                string.Empty,
                Ui.Pane.ColumnStatus,
                string.Empty,
                Ui.Pane.UseManageToAddOne)]);
        }
        else
        {
            SetItems(connections.Select(static connection => new BrowserListItem(
                ...
                string.Empty,
                connection.IsFavorite ? Ui.Pane.Favorite : Ui.Pane.Saved,
                connection.ConnectionId.ToString("D"),
                IsContainer: true)));
        }

        _connectionState.Text = Ui.Pane.StateChooseConnection;
    ...
    private static StorageProviderKind MapProvider(StorageConnectionProvider provider) => provider switch
    {
        ...
        _ => throw new ArgumentOutOfRangeException(nameof(provider), provider, Ui.Pane.UnknownProvider)
    };
    ...
    private void PresentSnapshot(LocalBrowserSnapshot snapshot, bool appendPage = false)
    {
        ...
        _connectionState.Text = Ui.Pane.StateReady;
        _connectionState.ForeColor = StorageHubTheme.Success;
        _connectionState.AccessibleDescription = Ui.Format(Ui.Pane.ShowingLocationFormat, snapshot.Location.DisplayText);
    ...
    private void SetNavigationBusy(bool isBusy)
    {
        if (isBusy)
        {
            _connectionState.Text = Ui.Pane.StateLoading;
        ...
        if (isBusy)
        {
            ...
            _summary.Text = Ui.Pane.Loading;
        }
        else
        {
            UpdateSummaryText();
        }
        _refreshButton.ToolTipText = isBusy ? Ui.Pane.CancelLoadAndRefresh : Ui.Pane.Refresh;
    }
    ...
    private void UpdateNavigationButtons()
    {
        ...
        _addressBox.AccessibleDescription = localActive
            ? Ui.Pane.LocalAddressHint
            : remoteActive
                ? Ui.Pane.RemoteAddressHint
                : Ui.Pane.ChooseConnection;
    ...
    private void ShowError(string message)
    {
        ...
        _connectionState.Text = Ui.Pane.StateLocationUnavailable;
    ...
    // Compared at runtime rather than as a constant pattern: the card's name is the text shown in
    // the selector, so it follows the current language and cannot be a compile-time constant.
    private bool IsLocalConnectionSelected =>
        _localBrowsingEnabled &&
        _connectionSelector.SelectedItem is ConnectionCardModel local &&
        string.Equals(local.Name, Ui.Pane.ThisPc, StringComparison.Ordinal);
    ...
    private bool IsLocalConnectionsHomeSelected =>
        _localBrowsingEnabled &&
        _connectionSelector.SelectedItem is ConnectionCardModel home &&
        string.Equals(home.Name, Ui.Pane.ConnectionsHome, StringComparison.Ordinal);
    ...
    private void AddressBoxKeyDown(object? sender, KeyEventArgs e)
    {
        ...
        if (IsLocalConnectionSelected)
        {
            ...
            {
                ShowError(errorMessage ?? Ui.Pane.InvalidFolderPath);
                return;
            }
        ...
        {
            ShowError(remoteError ?? Ui.Pane.InvalidRemotePath);
            return;
        }
    ...
    private async void FileListItemDrag(object? sender, ItemDragEventArgs e)
    {
        ...
        else if (selection.Value.Context.Kind == PaneTransferContextKind.SavedConnection && BeginExplorerDropAsync is not null)
        {
            ...
            {
                ShowError(Ui.Pane.DropCouldNotInitialize);
                return;
            }
        ...
        catch (Exception error) when (error is ExternalException or InvalidOperationException)
        {
            ...
            if (dropToken is not null) PendingDrops?.MarkFailed(dropToken, Ui.Pane.DragCouldNotStart);
            ShowError(Ui.Format(Ui.Pane.DragCouldNotStartFormat, error.Message));
            return;
        }
        ...
        if (registration is not null && CommitExplorerDropAsync is not null)
        {
            try
            {
                ...
                if (registered.Failure is not null || string.IsNullOrWhiteSpace(registered.DropToken))
                {
                    ...
                    PendingDrops?.MarkFailed(
                        dropToken!,
                        registered.Failure?.Message ?? Ui.Pane.DropNotInitialized);
                    if (!payload.InternalDropHandled)
                    {
                        ShowError(registered.Failure?.Message ?? Ui.Pane.DropCouldNotInitialize);
                    }
                ...
                else
                {
                    PendingDrops?.MarkCancelled(
                        dropToken!,
                        committed.Failure?.Message ?? Ui.Pane.NoDestinationReported);
                }

                if (!payload.InternalDropHandled && committed.Accepted)
                {
                    _errorBanner.Text = Ui.Format(Ui.Pane.QueuedToFormat, committed.DestinationPath);
                ...
                InvalidOperationException or TimeoutException or System.Text.Json.JsonException or OperationCanceledException)
            {
                ...
                if (!payload.InternalDropHandled)
                    ShowError(Ui.Format(Ui.Pane.DropCouldNotQueueFormat, error.Message));
            }
            finally
            {
                registrationTimeout?.Dispose();
    ...
    /// <summary>
    /// Builds one context-menu row, optionally mirroring a menu command's keyboard shortcut.
    /// </summary>
    /// <remarks>
    /// The command is named rather than inferred from <paramref name="text"/>. Matching on the
    /// visible text meant trimming dots, ignoring case and special-casing Ui.Pane.Move to find Ui.Pane.Cut,
    /// and it would stop working entirely the moment either side was translated. The shortcut is
    /// now read from the catalog too, so a rebound key shows here as well.
    /// </remarks>
    private ToolStripMenuItem CreateContextMenuItem(string text, UiGlyph glyph, string? commandId = null)
    {
        ...
        return new ToolStripMenuItem(text)
        {
            Image = image,
            Tag = commandId,
            ShortcutKeyDisplayString = commandId is null ? null : ShortcutDisplay(commandId),
            AccessibleName = text
        };
    }

    /// <summary>
    /// The shortcut as the user would press it: their own binding when the pane is hosted by the
    /// shell, and the command's default otherwise.
    /// </summary>
    ...
    private static void RefreshContextShortcuts(ToolStripItemCollection items, MainForm main)
    {
        foreach (var item in items.OfType<ToolStripMenuItem>())
            if (item.Tag is string commandId) item.ShortcutKeyDisplayString = main.ShortcutDisplay(commandId);
    }
    ...
    private StorageResult<PaneTransferContext> CaptureTransferContext()
    {
        if (IsSshClientSelected)
        {
            return StorageResult<PaneTransferContext>.Fail(new StorageFailure(
                ...
                Ui.Pane.ChooseFolderBeforeTransfer));
        }
    ...
    private ToolStripDropDownButton CreateMoreCommandsButton(string paneTitle)
    {
        ...
        var button = new ToolStripDropDownButton
        {
            ...
            AccessibleDescription = Ui.Pane.MoreCommandsHint,
            ToolTipText = Ui.Pane.MoreFileCommands,
        ...
        var batchRename = CreateContextMenuItem(Ui.Pane.BatchRename, UiGlyph.File, UiCommandIds.EditBatchRename);
        var copy = CreateContextMenuItem(Ui.Pane.Copy, UiGlyph.File, UiCommandIds.EditCopy);
        var move = CreateContextMenuItem(Ui.Pane.Move, UiGlyph.Forward, UiCommandIds.EditCut);
        var paste = CreateContextMenuItem(Ui.Pane.Paste, UiGlyph.Save, UiCommandIds.EditPaste);
        var delete = CreateContextMenuItem(Ui.Pane.Delete, UiGlyph.Delete, UiCommandIds.EditDelete);
        var refresh = CreateContextMenuItem(Ui.Pane.Refresh, UiGlyph.Refresh, UiCommandIds.ViewRefresh);
        var selectAll = CreateContextMenuItem(Ui.Pane.SelectAll, UiGlyph.Test, UiCommandIds.EditSelectAll);
        var properties = CreateContextMenuItem(Ui.Pane.Properties, UiGlyph.Info, UiCommandIds.EditProperties);
        var invertSelection = CreateContextMenuItem(Ui.Pane.InvertSelection, UiGlyph.Test, UiCommandIds.EditInvertSelection);
    ...
    private TableLayoutPanel CreateLoadingOverlay()
    {
        var overlay = new TableLayoutPanel
        {
            ...
            RowCount = 3,
            Visible = false,
            AccessibleName = Ui.Pane.FolderLoadingIndicator,
    ...
    private static TableLayoutPanel CreateEmptyStateOverlay(out Label message)
    {
        var overlay = new TableLayoutPanel
        {
            ...
            RowCount = 3,
            Visible = false,
            AccessibleName = Ui.Pane.EmptyFolderNotice
        };
        overlay.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        overlay.RowStyles.Add(new RowStyle(SizeType.Percent, 45));
        overlay.RowStyles.Add(new RowStyle(SizeType.AutoSize));
    ...
    private Label CreatePaneBadge(string text, Color color)
    {
        ...
        // Drawn as a tinted pill rather than left as a muted rectangle. Against the flat header
        // the old background was the same colour as what sat behind it, so the badges read as
        // loose words rather than labels.
        badge.Paint += (sender, args) =>
        {
            if (sender is not Label painted)
            {
                return;
            }

            args.Graphics.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
            var bounds = new RectangleF(0.5F, 0.5F, painted.Width - 1F, painted.Height - 1F);
            using var shape = UiShapes.RoundedRectangle(bounds, bounds.Height / 2F);
            using var fill = new SolidBrush(Color.FromArgb(38, painted.ForeColor));
            using var edge = new Pen(Color.FromArgb(95, painted.ForeColor));
            args.Graphics.FillPath(fill, shape);
            args.Graphics.DrawPath(edge, shape);
            TextRenderer.DrawText(
                args.Graphics,
                painted.Text,
                painted.Font,
                painted.ClientRectangle,
                painted.ForeColor,
    ...
    private static string GetConnectionGroupLabel(ConnectionCardModel card)
    {
        ...
        {
            return Ui.Pane.OnThisDevice;
        }

        if (!card.IsEnabled)
        {
            return Ui.Pane.Disabled;
        }

        if (card.IsFavorite)
        {
            return Ui.Pane.Favorites;
        }
    ...
    private void UpdateConnectionPresentation()
    {
        ...
        _connectionNameLabel.AccessibleDescription = Ui.Format(Ui.Pane.ActiveConnectionFormat, card.Name);
        ...
        _connectionState.Text = clientActive
            ? Ui.Pane.StateSshTerminal
            : localActive || remoteActive
                ? Ui.Pane.StateReady
                : Ui.Pane.StateChooseConnection;
