...
public sealed class ScheduleManagerForm : Form
{
    ...
    private readonly StorageHubChoiceField _profile;
    private readonly StorageHubChoiceField _frequency;
    private readonly StorageHubTimeField _scheduleTime;
    private readonly StorageHubChoiceField _weekDay;
    private readonly StorageHubNumberField _monthDay;
    private readonly StorageHubTextField _cron;
    ...
    private readonly StorageHubChoiceField _timeZone;
    private readonly StorageHubNumberField _misfireGraceMinutes;
    private readonly StorageHubCheckBox _queueOne;
    private readonly StorageHubCheckBox _enabled;
    private readonly StorageHubChoiceField _executionMode;
    ...
    private readonly StorageHubButton _toggle;
        ...
        bool ownsClients = false)
    {
        ...
        var refresh = new StorageHubButton { Text = Ui.Schedules.Refresh, AutoSize = true };
        refresh.Variant = StorageHubButtonVariant.Secondary;
        refresh.Click += RefreshClicked;
        var newSchedule = new StorageHubButton { Text = Ui.Schedules.NewSchedule, AutoSize = true };
        newSchedule.Variant = StorageHubButtonVariant.Primary;
        ...
        _grid = new DataGridView
        {
            ...
            AccessibleName = Ui.Schedules.SavedSchedules
        };
        ...
        _grid.Columns.Add(Ui.Schedules.Profile, Ui.Schedules.Profile);
        _grid.Columns.Add(Ui.Schedules.Cron, Ui.Schedules.Schedule);
        _grid.Columns.Add("TimeZone", Ui.Schedules.TimeZone);
        _grid.Columns.Add(Ui.Schedules.Next, Ui.Schedules.NextRun);
        _grid.Columns.Add(Ui.Schedules.Enabled, Ui.Schedules.Enabled);
        _grid.Columns.Add(Ui.Schedules.State, Ui.Schedules.State);
        ...
        [
            new FrequencyChoice(ScheduleFrequency.Daily, Ui.Schedules.EveryDay),
            new FrequencyChoice(ScheduleFrequency.Weekdays, Ui.Schedules.EveryWeekday),
            new FrequencyChoice(ScheduleFrequency.Weekly, Ui.Schedules.EveryWeek),
            new FrequencyChoice(ScheduleFrequency.Monthly, Ui.Schedules.EveryMonth),
            new FrequencyChoice(ScheduleFrequency.Custom, Ui.Schedules.CustomAdvanced)
        ]);
        ...
        _monthDay = new StorageHubNumberField
        {
        ...
        _cron = new StorageHubTextField
        {
            ...
            AccessibleName = Ui.Schedules.CronField
        };
        ...
        _misfireGraceMinutes = new StorageHubNumberField
        {
        ...
        {
            Text = Ui.Schedules.CoalesceOption,
            Checked = true,
            AutoSize = true
        };
        _enabled = new StorageHubCheckBox
        {
            Text = Ui.Schedules.Enabled,
            Checked = false,
            AutoSize = true
        };
        _executionMode = new StorageHubChoiceField
        {
            ...
            // flag is set: the previous handler had never once run, so the list showed the enum's
            // own names for as long as this dialog has existed.
            DisplayText = DescribeExecutionMode
        };
        ...
        _nextOccurrence = UiControlFactory.CreateDescription(Ui.Schedules.NotScheduledUntilSaved);
        _runState = UiControlFactory.CreateDescription(Ui.Schedules.NoActiveRun);
        ...
        {
            Name = "ScheduleStatus",
            Text = Ui.Schedules.ManagerConnectsWhenShown,
            AutoSize = true,
            Anchor = AnchorStyles.Left,
            ForeColor = StorageHubTheme.TextMuted,
            AccessibleName = Ui.Schedules.ManagementStatus
        };
        footer.Controls.Add(_status, 0, 0);
        var actions = new FlowLayoutPanel { AutoSize = true, WrapContents = false, Margin = Padding.Empty };
        var close = new StorageHubButton { Text = Ui.Schedules.Close, DialogResult = DialogResult.Cancel };
        close.Variant = StorageHubButtonVariant.Secondary;
        _delete = new StorageHubButton { Name = "DeleteSchedule", Text = Ui.Schedules.Delete, AutoSize = true, Enabled = false };
        _delete.Variant = StorageHubButtonVariant.Secondary;
        _delete.Click += DeleteClicked;
        _toggle = new StorageHubButton { Name = "ToggleSchedule", Text = Ui.Schedules.Enable, AutoSize = true, Enabled = false };
        _toggle.Variant = StorageHubButtonVariant.Secondary;
        _toggle.Click += ToggleClicked;
        _save = new StorageHubButton { Name = "SaveSchedule", Text = Ui.Schedules.SaveSchedule, AutoSize = true };
        _save.Variant = StorageHubButtonVariant.Primary;
    ...
    public async Task LoadSchedulesAsync(CancellationToken cancellationToken = default)
    {
        ...
        _status.Text = Ui.Schedules.LoadingSchedules;
        ...
        _status.Text = schedules.Schedules.Length == 0
            ...
            : Ui.Format(Ui.Schedules.SchedulesLoadedFormat, schedules.Schedules.Length);
    ...
    public async Task SelectScheduleAsync(Guid scheduleId, CancellationToken cancellationToken = default)
    {
        ...
        ApplySchedule(response.Schedule ?? throw new InvalidDataException(Ui.Schedules.IncompleteSchedule));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException(Ui.Schedules.CronHint);
        }
    ...
    public async Task<ScheduleDocument> SetCurrentEnabledAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var current = _current ?? throw new InvalidOperationException(Ui.Schedules.SelectScheduleFirst);
        ...
        _status.Text = enabled
            ? Ui.Schedules.ScheduleEnabled
            : Ui.Schedules.ScheduleDisabled;
        _status.ForeColor = StorageHubTheme.Success;
        return updated;
    }
    ...
    public async Task DeleteCurrentScheduleAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException(response.Failure?.Message ?? Ui.Schedules.ScheduleDeleteFailed);
        }

        RemoveGridRow(current.ScheduleId);
        BeginNewSchedule();
        _status.Text = Ui.Schedules.ScheduleDeleted;
        _status.ForeColor = StorageHubTheme.Success;
    }
    ...
    private Panel BuildEditor()
    {
        ...
        table.RowCount = 2;
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.SyncProfile, _profile, Ui.Schedules.OnlySavedProfiles);
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.RunSchedule, _scheduleBuilder, Ui.Schedules.CustomCronOnly);
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.TimeZone, _timeZone, Ui.Schedules.TimeZoneHint);
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.MisfireGrace, _misfireGraceMinutes, Ui.Schedules.MisfireHint);
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.Overlap, _queueOne, Ui.Schedules.CoalesceHint);
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.Enabled, _enabled, Ui.Schedules.NewSchedulesDisabled);
        UiControlFactory.AddLabeledRow(
            table,
            Ui.Schedules.ExecutionMode,
            _executionMode,
            Ui.Schedules.SafeAutomaticHint);
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.NextRun, _nextOccurrence, Ui.Schedules.CalculatedAfterSave);
        UiControlFactory.AddLabeledRow(table, Ui.Schedules.CurrentState, _runState, Ui.Schedules.ActiveRunsBlock);
    ...
    private TableLayoutPanel BuildScheduleBuilder()
    {
        ...
        AddBuilderRow(builder, 3, Ui.Schedules.Day, _monthDay, "MonthDayLabel");
        AddBuilderRow(builder, 4, Ui.Schedules.Cron, _cron, "CronLabel");
        return builder;
    }
    ...
    private static string DescribeExecutionMode(object item) => item switch
    {
        ScheduleIpcExecutionMode.SafeAutomatic => Ui.Schedules.SafeAutomatic,
        ScheduleIpcExecutionMode.PreviewOnly => Ui.Schedules.ReviewOnly,
    ...
    private void UpdateExecutionModeNotice()
    {
        ...
        _modeNotice.Text = automatic
            ? Ui.Schedules.SafeAutomaticDescription
            : Ui.Schedules.ReviewOnlyDescription;
    ...
    private static string DescribeSchedule(string expression)
    {
        ...
        if (fields.Length == 5 && int.TryParse(fields[0], out var minute) && int.TryParse(fields[1], out var hour))
        {
            ...
            if (fields[2] == "*" && fields[3] == "*" && fields[4] == "*") return Ui.Format(Ui.Schedules.RunsDailyFormat, time);
            if (fields[2] == "*" && fields[3] == "*" && fields[4] == "1-5") return Ui.Format(Ui.Schedules.RunsWeekdaysFormat, time);
            if (fields[2] == "*" && fields[3] == "*" && int.TryParse(fields[4], out var day) && day is >= 0 and <= 6)
                return Ui.Format(Ui.Schedules.RunsWeeklyFormat, (DayOfWeek)day, time);
            if (int.TryParse(fields[2], out var monthDay) && fields[3] == "*" && fields[4] == "*")
                return Ui.Format(Ui.Schedules.RunsMonthlyFormat, monthDay, time);
        }

        return Ui.Schedules.AdvancedRecurrence;
    }
    ...
    private void BeginNewSchedule()
    {
        ...
        _nextOccurrence.Text = Ui.Schedules.NotScheduledUntilSaved;
        _runState.Text = Ui.Schedules.NoActiveRun;
        _toggle.Text = Ui.Schedules.Enable;
        _toggle.Enabled = false;
        _delete.Enabled = false;
        _save.Enabled = true;
        _grid.ClearSelection();
        _status.Text = Ui.Schedules.NewScheduleDraft;
        _status.ForeColor = StorageHubTheme.TextMuted;
    }
    ...
    private void ApplySchedule(ScheduleDocument schedule)
    {
        ...
        _nextOccurrence.Text = schedule.NextOccurrenceUtc is { } next
            ...
            : Ui.Schedules.NoFutureRun;
        _runState.Text = schedule.IsBusy
            ? Ui.Schedules.RunActiveBlocks
            : schedule.LastRunOutcome is { Length: > 0 } outcome
                ? Ui.Format(Ui.Schedules.IdleOutcomeFormat, outcome, FormatErrorCode(schedule.LastErrorCode))
                : Ui.Schedules.IdleNoOutcome;
        _toggle.Text = schedule.Enabled ? Ui.Schedules.Disable : Ui.Schedules.Enable;
    ...
    private void AddGridRow(ScheduleDocument schedule)
    {
        var index = _grid.Rows.Add(
            ...
            schedule.Enabled ? Ui.Schedules.Yes : "No",
            schedule.IsBusy ? Ui.Schedules.RunActive : schedule.LastRunOutcome ?? Ui.Schedules.Idle);
    ...
    private static string FormatTimeZone(string timeZoneId)
    {
        ...
        {
            return Ui.Format(Ui.Schedules.UnavailableRegionFormat, timeZoneId);
        }
    }
    ...
    private async void RefreshClicked(object? sender, EventArgs e)
    {
        await RunBusyAsync(
            ...
            Ui.Schedules.RefreshingSchedules).ConfigureAwait(true);
    }
    ...
    private async void SaveClicked(object? sender, EventArgs e)
    {
        await RunBusyAsync(
            ...
            Ui.Schedules.SavingSchedule).ConfigureAwait(true);
    }
    ...
    private async void ToggleClicked(object? sender, EventArgs e)
    {
        ...
        await RunBusyAsync(
            ...
            target ? Ui.Schedules.EnablingSchedule : Ui.Schedules.DisablingSchedule).ConfigureAwait(true);
    }
    ...
    private async void DeleteClicked(object? sender, EventArgs e)
    {
        if (MessageBox.Show(
                this,
                Ui.Dialogs.DeleteSchedulePrompt,
                Ui.Dialogs.DeleteScheduleCaption,
        ...
        await RunBusyAsync(
            ...
            Ui.Schedules.DeletingSchedule).ConfigureAwait(true);
    }
    ...
    private static ScheduleDocument RequireSuccessfulSchedule(ScheduleMutationResponse response)
    {
        if (response.Outcome is not (ScheduleMutationOutcome.Succeeded or ScheduleMutationOutcome.AlreadyApplied) ||
            response.Failure is not null)
        {
            throw new InvalidOperationException(response.Failure?.Message ?? Ui.Schedules.ScheduleChangeFailed);
        }

        return response.Schedule ?? throw new InvalidDataException(Ui.Schedules.NoScheduleRevision);
    }
    ...
    private sealed record TimeZoneChoice(string Id, string DisplayName)
    {
        ...
        public static TimeZoneChoice Unavailable(string id) => new(id, Ui.Format(Ui.Schedules.UnavailableRegionFormat, id));
