...
public static class StorageHubTheme
{
    ...
    /// <summary>The fill inside an input, and the text colour of a control that cannot be used.</summary>
    public static Color Input => CurrentPalette.Input;
    public static Color DisabledText => CurrentPalette.DisabledText;

    /// <summary>The accent under a pointer and under a press.</summary>
    ...
    public static void ReduceFlicker(Control control)
    {
        ...
        var property = DoubleBufferedProperties.GetOrAdd(
            control.GetType(),
            static type => type.GetProperty(
                "DoubleBuffered",
                System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic));
        if (property is not null && property.GetValue(control) is not true)
        {
    ...
    private static void ListResized(object? sender, EventArgs e)
    {
        if (sender is ListView list)
        {
            ...
            // header is created lazily, so it needs the later pass; the chrome call is a no-op
            // once the handles it has already themed are unchanged.
            FillListHeader(list);
            ApplyNativeChrome(list);
        }
    }
    ...
    /// <summary>
    /// Per-list bookkeeping for <see cref="FillListHeader"/>. A column assignment can add or
    /// remove a scrollbar, which changes the client size, which raises Resize, which calls the
    /// fill again from inside the first call. Left unguarded that nesting became a storm: with
    /// the rows nearly filling the height and the columns nearly filling the width, each pass
    /// flipped one scrollbar and the next pass flipped it back, and the overview page spent half
    /// a minute at full CPU repainting cards before the native control gave up.
    /// </summary>
    private sealed class ListFillState
    {
        public bool Filling { get; set; }
        public bool Pending { get; set; }
    }

    private static readonly System.Runtime.CompilerServices.ConditionalWeakTable<ListView, ListFillState> ListFills = new();

    /// <summary>The most passes one fill runs before accepting the layout as it stands.</summary>
    private const int MaximumFillPasses = 3;
    ...
    private static void FillListHeader(ListView list)
    {
        ...
        var state = ListFills.GetOrCreateValue(list);
        if (state.Filling)
        {
            // A nested call from the Resize this fill itself provoked: note it and let the outer
            // pass re-measure once it has unwound, rather than recursing.
            state.Pending = true;
            return;
        }
        ...
        try
        {
            for (var pass = 0; pass < MaximumFillPasses; pass++)
            {
                state.Pending = false;
                FillTrailingColumn(list);
                if (!state.Pending)
                {
                    break;
                }
    ...
    private static void FillTrailingColumn(ListView list)
    {
        ...
        {
            // The control re-evaluates its scroll range on a column assignment and at no other
            // time, so a horizontal scrollbar raised by an earlier, narrower layout survives any
            // pass that skips the write. Only that leftover scrollbar earns the nudge: writing
            // the same width on every resize repainted the header and the rows for nothing.
            if (!HasHorizontalScrollbar(list))
            {
                return;
            }
    ...
    private static bool HasHorizontalScrollbar(ListView list) =>
        list.IsHandleCreated && (GetWindowLong(list.Handle, GwlStyle) & WsHScroll) != 0;

    public static void ApplyPrimaryButtonSurface(Button button)
    {
    ...
    public static void ApplySecondaryButtonSurface(Button button)
    {
        var palette = CurrentPalette;
    ...
    public static void ApplyDangerButtonSurface(Button button)
    {
        var palette = CurrentPalette;
    ...
    public static void ConfigureTabs(TabControl tabs)
    {
        ...
        // Asked of the control, not inferred from its accessible name. That name is translated,
        // so the old comparison against "Workspace tabs" matched only in English: everywhere else
        // the shared renderer was attached on top of the owner's and, being re-subscribed by the
        // theme walk after construction, painted last over the icon and the close cross.
        var ownsItemRendering = tabs is ThemedTabControl { OwnsItemRendering: true };
        tabs.DrawMode = TabDrawMode.OwnerDrawFixed;
        tabs.DrawItem -= DrawTab;
        if (!ownsItemRendering)
        {
            tabs.DrawItem += DrawTab;
        }
    ...
    private static void DrawTab(object? sender, DrawItemEventArgs e)
    {
                   ...
                   5F))
        {
            ...
            // An unselected tab is outlined too. Drawing its edge in the same colour as its fill
            // is what made the separation invisible.
            using var outline = new Pen(selected ? palette.Border : palette.Canvas);
    ...
    internal static void ApplyNativeChrome(Control control)
    {
        ...
        // The column header is a comctl32 window rather than a WinForms child, so it is not
        // reached by walking Controls. Owner drawing covers the header cells but not the gap
        // past the last column, which stayed a bright block in dark mode.
        var header = control is ListView
            ? SendMessage(control.Handle, LvmGetHeader, IntPtr.Zero, IntPtr.Zero)
            : IntPtr.Zero;

        // A theme sticks to a window for as long as that window exists, so re-applying it to the
        // same handles is pure cost: SetWindowTheme raises WM_THEMECHANGED, and the control
        // repaints in full. This used to run on every Resize of every list, which was a flash
        // across each list on each step of a window drag.
        var signature = new NativeChromeSignature(control.Handle, header, ChildHandleSignature(control), theme);
        var state = NativeChromes.GetOrCreateValue(control);
        if (state.Applied == signature)
        {
            return;
        }
        ...
        try
        {
            ...
            // Double buffering for the two native controls that flash on a resize. It has to be
            // the control's own extended style: the managed DoubleBuffered property makes
            // WinForms intercept WM_PAINT for a window that comctl32 paints itself, and an
            // owner-drawn list then renders as unpainted black blocks.
            if (!SystemInformation.TerminalServerSession)
            {
                if (control is TreeView)
                {
                    _ = SendMessage(control.Handle, TvmSetExtendedStyle, TvsExDoubleBuffer, TvsExDoubleBuffer);
                }
                else if (control is ListView)
                {
                    _ = SendMessage(control.Handle, LvmSetExtendedStyle, LvsExDoubleBuffer, LvsExDoubleBuffer);
                }
            }
    ...
    private static long ChildHandleSignature(Control control)
    {
        // Enough to notice a child window appearing or being recreated; the walk is the same one
        // the theming does, and cheaper than the WM_THEMECHANGED it saves.
        var signature = 17L;
        foreach (Control child in control.Controls)
        {
            if (child.IsHandleCreated)
            {
                signature = unchecked((signature * 31) + child.Handle.ToInt64());
            }
        }
    ...
    private readonly record struct NativeChromeSignature(IntPtr Handle, IntPtr Header, long Children, string Theme);

    private sealed class NativeChromeState
    {
        public NativeChromeSignature? Applied { get; set; }
    }

    private static readonly System.Runtime.CompilerServices.ConditionalWeakTable<Control, NativeChromeState> NativeChromes = new();
    ...
    private static readonly IntPtr TvsExDoubleBuffer = 0x0004;
    private const int LvmSetExtendedStyle = 0x1000 + 54;
    private static readonly IntPtr LvsExDoubleBuffer = 0x00010000;
