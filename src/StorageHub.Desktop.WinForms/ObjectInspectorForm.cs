...
public sealed class ObjectInspectorForm : Form
{
    ...
    private readonly StorageHubButton _loadMoreButton;
        ...
        bool ownsController = false)
    {
        ...
        AccessibleName = Ui.Inspector.StorageObjectInspector;
        ...
        var menu = new MenuStrip
        {
            ...
            AccessibleName = Ui.Inspector.ObjectInspectorCommands
        };
        ...
        _loadMoreMenu = new ToolStripMenuItem(Ui.Inspector.LoadMoreVersions, null, LoadMoreClicked)
        {
            Enabled = false
        };
        ...
        _versionsGrid = CreateGrid(Ui.Inspector.ObjectVersions);
        _versionsGrid.Columns.Add(Ui.Inspector.Current, Ui.Inspector.Current);
        _versionsGrid.Columns.Add(Ui.Inspector.Version, Ui.Inspector.VersionID);
        _versionsGrid.Columns.Add("Size", "Size");
        _versionsGrid.Columns.Add(Ui.Inspector.Modified, Ui.Inspector.ModifiedUTC);
        _versionsGrid.Columns.Add("DeleteMarker", Ui.Inspector.DeleteMarker);
        _versionsGrid.Columns.Add("EntityTag", Ui.Inspector.EntityTag);
        ...
        _metadataGrid = CreateGrid(Ui.Inspector.ObjectMetadata);
        ...
        _tagsGrid = CreateGrid(Ui.Inspector.ObjectTags);
        ...
        _versionsNotice = CreateNotice(Ui.Inspector.VersionHistoryHasNotBeenLoaded);
        _metadataNotice = CreateNotice(Ui.Inspector.MetadataHasNotBeenLoaded);
        _tagsNotice = CreateNotice(Ui.Inspector.TagsHaveNotBeenLoaded);
        _loadMoreButton = new StorageHubButton
        {
            Name = "LoadMoreObjectVersions",
            Text = Ui.Inspector.LoadMoreVersions2,
            AutoSize = true,
            Enabled = false,
            AccessibleName = Ui.Inspector.LoadTheNextObjectVersionPage
        };
        _loadMoreButton.Variant = StorageHubButtonVariant.Secondary;
        ...
        _metadataTab = CreateDataTab(Ui.Inspector.Metadata, _metadataGrid, _metadataNotice);
        _tagsTab = CreateDataTab("Tags", _tagsGrid, _tagsNotice);
        var tabs = new ThemedTabControl
        {
            Name = "ObjectInspectorTabs",
            Dock = DockStyle.Fill,
            AccessibleName = Ui.Inspector.ObjectDetailCategories
        };
        ...
        _status = new Label
        {
            ...
            Dock = DockStyle.Fill,
            Text = Ui.Inspector.TheInspectorConnectsToTheBackgroundAgent,
            ForeColor = StorageHubTheme.TextMuted,
            AutoEllipsis = true,
            AccessibleName = Ui.Inspector.ObjectInspectorStatus
        };
    ...
    public async Task LoadInspectorAsync(CancellationToken cancellationToken = default)
    {
        using var linked = CancellationTokenSource.CreateLinkedTokenSource(
            cancellationToken,
            _lifetime.Token);
        SetBusy(Ui.Inspector.LoadingVersionHistoryMetadataAndTags);
    ...
    public async Task LoadMoreVersionsAsync(CancellationToken cancellationToken = default)
    {
        using var linked = CancellationTokenSource.CreateLinkedTokenSource(
            cancellationToken,
            _lifetime.Token);
        SetBusy(Ui.Inspector.LoadingTheNextVersionPage);
    ...
    private Panel BuildHeading(ObjectInspectorAddress address)
    {
        ...
        {
            Dock = DockStyle.Fill,
            Text = Ui.Inspector.READONLYVersionPagesMetadataAndTags,
            ForeColor = StorageHubTheme.Success,
            AccessibleName = Ui.Inspector.ReadOnlyInspectorSafetyNotice
        };
    ...
    private void ApplyState(ObjectInspectorState state)
    {
        ...
        _versionsTab.Text = Ui.Format(Ui.Inspector.TabVersionsFormat, state.Versions.Count);
        _metadataTab.Text = Ui.Format(Ui.Inspector.TabMetadataFormat, state.Metadata.Count);
        _tagsTab.Text = Ui.Format(Ui.Inspector.TabTagsFormat, state.Tags.Count);
        SetNotice(
            _versionsNotice,
            state.VersionsFailure,
            state.Versions.Count,
            Ui.Inspector.NoVersionsReturned,
            Ui.Inspector.LoadedVersionsFormat);
        SetNotice(
            _metadataNotice,
            state.MetadataFailure,
            state.Metadata.Count,
            Ui.Inspector.NoMetadataReturned,
            Ui.Inspector.LoadedMetadataFormat);
        SetNotice(
            _tagsNotice,
            state.TagsFailure,
            state.Tags.Count,
            Ui.Inspector.NoTagsReturned,
            Ui.Inspector.LoadedTagsFormat);
    ...
    /// <remarks>
    /// The empty and loaded sentences are passed in already localized rather than composed from a
        ...
        string loadedFormat)
    {
        label.Text = failure?.Message ??
            (count == 0 ? emptyText : Ui.Format(loadedFormat, count));
    ...
    private void ShowFailure(Exception error)
    {
        _status.Text = error switch
        {
            UnauthorizedAccessException =>
                Ui.Inspector.StorageHubCouldNotAuthenticateToTheLocal,
