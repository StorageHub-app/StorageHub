...
public static class SshTerminalIpcContract
{
    ...
    /// <summary>The longest a read may block waiting for output before answering empty.</summary>
    public const int MaximumReadWaitMilliseconds = 1_000;

    /// <summary>
    /// Exact equality, deliberately. The desktop and the agent ship in one bundle -- the agent is
    /// staged into the desktop's output by the desktop project -- so they are never independently
    /// versioned in the field. Negotiating a range would buy nothing and leave a compatibility
    /// matrix to maintain for ever; a mismatch means a stale agent is still running, and the
    /// desktop restarts it instead.
    /// </summary>
...
/// <param name="AcknowledgedSequence">
/// Everything at or below this has reached the desktop's emulator and the agent may discard it.
/// A read that was cancelled, timed out or threw simply never advances it, so the next read
/// replays the same bytes -- which is what makes a lost response harmless by construction.
/// </param>
/// <param name="WaitMilliseconds">
/// How long the agent may hold the read open waiting for output. Zero answers immediately.
/// </param>
public sealed record SshTerminalReadRequest(
    int ContractVersion,
    Guid SessionId,
    int MaximumBytes,
    ...
    int WaitMilliseconds = 0)
{
    public bool HasValidBounds =>
        ...
        WaitMilliseconds is >= 0 and <= SshTerminalIpcContract.MaximumReadWaitMilliseconds;
}

/// <param name="StartSequence">The absolute sequence the first returned byte sits at.</param>
/// <param name="Truncated">
/// The agent could not retain everything from the requested sequence, so output really was lost.
/// Saying so makes the gap visible instead of leaving a silent hole in the session.
/// </param>
