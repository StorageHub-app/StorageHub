...
public static class RemoteBrowserPath
{
    public static bool TryNormalize(string? value, out string path, out string? errorMessage)
    {
        ...
        if (value is null)
        {
            errorMessage = Ui.Validation.EnterAPathRelativeToTheConnection;
            return false;
        }
        ...
        {
            errorMessage = Ui.Validation.TheRemotePathIsTooLongOr;
            return false;
        }
        ...
        {
            errorMessage = Ui.Validation.EnterAPathRelativeToTheSelected;
            return false;
        }
        ...
        foreach (var segment in segments)
        {
            ...
            {
                errorMessage = Ui.Validation.ParentTraversalIsNotAllowedInA;
                return false;
            }
        ...
        {
            path = string.Empty;
            errorMessage = Ui.Validation.TheNormalizedRemotePathIsTooLong;
            return false;
        }
...
public sealed class RemoteBrowserController : IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (connection is null)
        {
            return new(
                RemoteBrowserOperationStatus.Failed,
                ErrorMessage: Ui.Validation.TheSavedConnectionIsNoLongerAvailable);
        }
                    ...
                    for (var depth = 0; depth < 256; depth++)
                    {
                        ...
                        if (fallback.Failure is null)
                        {
                            ...
                            return new(
                                ...
                                CurrentSnapshot,
                                Ui.Validation.ThatFolderNoLongerExistsStorageHubMoved,
                                missingPath);
                        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
                    ...
                    string.Equals(existing.RelativePath, next.RelativePath, StringComparison.Ordinal))))
            {
                return new(
                    RemoteBrowserOperationStatus.Failed,
                    ErrorMessage: Ui.Validation.TheProviderReturnedAnInconsistentNextPage);
            }
    ...
    public void Commit(RemoteBrowserNavigationKind kind, string path)
    {
        switch (kind)
        {
            ...
            case RemoteBrowserNavigationKind.Up:
                ...
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(kind), kind, Ui.Validation.UnknownRemoteNavigationKind);
        }
    }
}
...
internal static class RemoteBrowserErrors
{
    ...
    {
        TimeoutException => Ui.Validation.TheBackgroundAgentDidNotRespondIn,
        UnauthorizedAccessException => Ui.Validation.StorageHubCouldNotAuthenticateToTheLocal,
        IOException => Ui.Validation.TheBackgroundAgentIsUnavailable,
        InvalidDataException or JsonException => Ui.Validation.TheBackgroundAgentReturnedAnInvalidResponse,
    ...
    private static string ForCategory(StorageIpcFailure? failure) => failure?.Category switch
    {
        ...
        StorageIpcFailureCategory.Security =>
            Ui.Validation.TheSavedServerIdentityTrustDecisionIs,
        StorageIpcFailureCategory.Timeout => Ui.Validation.TheRemoteProviderDidNotRespondIn,
        StorageIpcFailureCategory.Cancelled => Ui.Validation.TheRemoteRequestWasCancelled,
        StorageIpcFailureCategory.Unsupported => Ui.Validation.ThisProviderDoesNotSupportTheRequested,
        StorageIpcFailureCategory.Integrity => Ui.Validation.TheProviderReturnedAnInvalidListing,
        StorageIpcFailureCategory.Unavailable => Ui.Validation.TheRemoteProviderIsTemporarilyUnavailable,
        _ => Ui.Validation.TheRemoteLocationCouldNotBeOpened
    };
}
