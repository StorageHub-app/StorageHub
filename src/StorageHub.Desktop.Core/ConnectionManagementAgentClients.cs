...
public interface IRemoteConnectionProfileClient : IAsyncDisposable
{
    ...
    Task<ConnectionSshHostKeyDiscoveryResponse> DiscoverSshHostKeyAsync(
        ...
        CancellationToken cancellationToken = default) =>
        throw new NotSupportedException(Ui.Validation.ThisProfileClientDoesNotSupportSSH);
...
public sealed class NamedPipeRemoteConnectionProfileClient : IRemoteConnectionProfileClient
{
        ...
        where TResponse : class
    {
        ...
        {
            throw new TimeoutException(Ui.Validation.TheProfileRequestTimedOutBeforeIt, error);
        }
        ...
        catch (OperationCanceledException error) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            throw new TimeoutException(Ui.Validation.TheProfileRequestTimedOut, error);
        }
    ...
    private static void EnsureRequest(int contractVersion, bool hasValidBounds, string parameterName)
    {
        if (!ConnectionProfileIpcContract.IsSupported(contractVersion) || !hasValidBounds)
        {
            throw new ArgumentException(
                Ui.Validation.TheProfileRequestIsOutsideTheConnection,
                parameterName);
        }
    }
    ...
    private static void EnsureTrustRequest(int contractVersion, bool hasValidBounds, string parameterName)
    {
        ...
        {
            throw new ArgumentException(
                Ui.Validation.TheTrustRequestIsOutsideTheConnection,
                parameterName);
        }
    }
    ...
    private static InvalidDataException InvalidResponse() =>
        new(Ui.Validation.TheLocalAgentReturnedAProfileResponse);

    private static InvalidDataException InvalidTrustResponse() =>
        new(Ui.Validation.TheLocalAgentReturnedATrustResponse);
...
public sealed class NamedPipeRemoteSecretVaultClient : IRemoteSecretVaultClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new ArgumentException(Ui.Validation.TheSecretRequestIsOutsideTheSecret, nameof(request));
        }
        ...
        {
            throw new TimeoutException(Ui.Validation.TheSecretRequestTimedOutBeforeIt, error);
        }
        ...
        {
            await DisconnectAfterFailureAsync().ConfigureAwait(false);
            throw new TimeoutException(Ui.Validation.TheSecretRequestTimedOut, error);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            await DisconnectAfterFailureAsync().ConfigureAwait(false);
    ...
    private static InvalidDataException InvalidResponse() =>
        new(Ui.Validation.TheLocalAgentReturnedASecretResponse);
