...
internal static class SettingsAgentTransfer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in imported)
        {
            ...
            if (match is not null && policy == SettingsConflictPolicy.Skip)
            {
                ...
                outcomes.Add(new SettingsItemOutcome(name, Ui.Validation.AlreadyHereLeftAlone));
                continue;
            }
        ...
        foreach (var entry in imported)
        {
                ...
                !map.TryResolveConnection(entry.Draft.RightConnectionId, connectionIds, out var right))
            {
                outcomes.Add(new SettingsItemOutcome(
                    entry.Draft.DisplayName, Ui.Validation.SkippedItsConnectionsAreNotHere));
                continue;
            }
            ...
            if (match is not null && policy == SettingsConflictPolicy.Skip)
            {
                ...
                outcomes.Add(new SettingsItemOutcome(draft.DisplayName, Ui.Validation.AlreadyHereLeftAlone));
                continue;
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in imported)
        {
            ...
            {
                outcomes.Add(new SettingsItemOutcome(name, Ui.Validation.SkippedItsSyncTaskIsNotHere));
                continue;
            }
    ...
    private static string Describe(SyncProfileMutationResponse response) => response.Outcome switch
    {
        ...
        SyncProfileMutationOutcome.RevisionConflict => Ui.Validation.SkippedItChangedWhileImporting,
        SyncProfileMutationOutcome.ConstraintConflict => Ui.Validation.SkippedItConflictsWithOneAlreadyHere,
        SyncProfileMutationOutcome.Unavailable => Ui.Validation.SkippedTheAgentWasUnavailable,
    ...
    private static string Describe(ScheduleMutationResponse response) => response.Outcome switch
    {
        ...
        ScheduleMutationOutcome.RevisionConflict => Ui.Validation.SkippedItChangedWhileImporting,
        ScheduleMutationOutcome.ActiveRun => Ui.Validation.SkippedItIsRunningNow,
        ScheduleMutationOutcome.ConstraintConflict => Ui.Validation.SkippedItConflictsWithOneAlreadyHere,
        ScheduleMutationOutcome.Unavailable => Ui.Validation.SkippedTheAgentWasUnavailable,
        _ => response.Failure?.Message ?? "Skipped"
    };

    private static string Describe(ConnectionProfileWriteStatus status) => status switch
    {
        ConnectionProfileWriteStatus.NameConflict => Ui.Validation.SkippedThatNameIsTaken,
        ConnectionProfileWriteStatus.VersionConflict => Ui.Validation.SkippedItChangedWhileImporting,
        ConnectionProfileWriteStatus.ValidationFailed => Ui.Validation.SkippedItIsNotValidHere,
        ConnectionProfileWriteStatus.Unavailable => Ui.Validation.SkippedTheAgentWasUnavailable,
        _ => "Skipped"
    };
