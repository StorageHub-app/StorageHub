...
internal sealed class SettingsImportService
{
    ...
    internal static SettingsFileInspection Inspect(string path)
    {
        ...
        try
        {
            ...
            if (!file.Exists)
            {
                return Unreadable(Ui.Validation.ThatFileNoLongerExists);
            }

            if ((file.Attributes & FileAttributes.ReparsePoint) != 0)
            {
                return Unreadable(Ui.Validation.StorageHubWillNotReadSettingsThroughA);
            }

            if (file.Length <= 0)
            {
                return Unreadable(Ui.Validation.ThatFileIsEmpty);
            }

            if (file.Length > SettingsExportEnvelopeLimits.MaximumFileBytes)
            {
                return Unreadable(Ui.Validation.ThatFileIsTooLargeToBe);
            }
        ...
        {
            return Unreadable(Ui.Validation.ThatFileCouldNotBeOpened);
        }
    }
    ...
    internal static SettingsExportReadResult Open(string path, string? password)
    {
        ...
        catch (Exception error) when (error is IOException or UnauthorizedAccessException or NotSupportedException)
        {
            return new SettingsExportReadResult(
                null, SettingsExportReadFailure.NotAnExport, Ui.Validation.ThatFileCouldNotBeOpened);
        }
        ...
        if (content.Length > SettingsExportEnvelopeLimits.MaximumFileBytes)
        {
            return new SettingsExportReadResult(
                ...
                Ui.Validation.ThatFileIsTooLargeToBe);
        }
        ...
        {
            return new SettingsExportReadResult(
                null, SettingsExportReadFailure.NotAnExport, Ui.Validation.ThisFileNeedsAPassword);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        else if (agentSections.Length > 0)
        {
            foreach (var id in agentSections)
            {
                blocked[id] = Ui.Validation.TheBackgroundAgentIsNotRunning;
            }
        }
