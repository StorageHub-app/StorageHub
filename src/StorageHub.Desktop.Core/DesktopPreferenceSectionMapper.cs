...
internal static class DesktopPreferenceSectionMapper
{
        ...
        IReadOnlyCollection<SettingsSectionId> chosen)
    {
        ...
        if (selected.Contains(SettingsSectionId.DesktopGeneral) && document.DesktopGeneral is { } general)
        {
            ...
            if (DesktopConfigRepair.Validate(candidate) is { } error)
            {
        ...
        {
            // Validated before resolving. ShortcutBindings.Resolve silently falls back to the full
            // default set when given an invalid map, which during an import would wipe every
            // shortcut the user had without a word.
            var bindings = ShortcutChord.Parse(shortcuts);
            if (ShortcutBindings.Validate(bindings) is { } error)
            {
                blocked[SettingsSectionId.Shortcuts] = error;
            }
            else
            {
                updated = updated with { Shortcuts = ShortcutBindings.Resolve(bindings) };
    ...
    private static DesktopUpdatePreferences ApplyGeneral(
        ...
        DesktopGeneralSection general) => current with
        {
            ...
            ShowFavoritesInTheirFolders = general.ShowFavoritesInTheirFolders,
            // Sanitised on import: a file from another machine may name a command this build
            // no longer has, which must drop the entry rather than the import.
            ToolbarItems = general.ToolbarItems is null
        ...
        Dictionary<SettingsSectionId, string> blocked)
    {
        ...
        {
            if (DesktopConfigRepair.IsValidEditorPath(imported))
            {
                editorPath = imported;
            }
            else
