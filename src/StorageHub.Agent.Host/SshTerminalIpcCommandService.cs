...
public sealed class SshTerminalIpcCommandService : IAgentIpcCommandHandler, IAsyncDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Checked before the bounds check, because HasValidBounds folds the version in: without
        // this a stale agent answers a generic "invalid request" and the desktop has nothing to
        // act on. With it, the desktop knows to restart the agent and retry.
        if (!SshTerminalIpcContract.IsSupported(request.ContractVersion))
        {
            return OpenFailure("ssh.terminal.version.mismatch", StorageIpcFailureCategory.Validation,
                "The SSH terminal agent is running an out-of-date protocol version.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        // The ack only ever arrives for bytes the desktop has already fed to its emulator, so
        // discarding them here cannot lose anything that was still in flight.
        session.Output.Acknowledge(request.AcknowledgedSequence);

        if (request.WaitMilliseconds > 0 && session.IsConnected)
        {
            await session.Output.WaitForDataAsync(
                request.AcknowledgedSequence,
                TimeSpan.FromMilliseconds(request.WaitMilliseconds),
                cancellationToken).ConfigureAwait(false);
        }

        var slice = session.Output.Read(request.AcknowledgedSequence, request.MaximumBytes);
        var connected = session.IsConnected;

        // A faulted session keeps answering until its ring is empty. Only then is it removed, so
        // the last of what the remote said still reaches the screen.
        if (!connected && session.IsDrained)
        {
            RemoveSession(request.SessionId);
        }
    ...
    private void ScavengeIdleSessions()
    {
        ...
        // A faulted session still holding output is not idle -- it is waiting to be drained, and
        // reaping it here would discard exactly the bytes the ring exists to preserve.
        foreach (var pair in _sessions
            .Where(pair => pair.Value.LastActivityUtc < cutoff && pair.Value.Output.BufferedBytes == 0)
            .ToArray())
        {
    ...
    private sealed class SshTerminalSession : IDisposable
    {
        ...
        internal SshTerminalSession(SshClient client, ShellStream shell, PrivateKeyFile? authenticationResource)
        {
            ...
            // A continuous pump, not a read per poll. Pulling only when asked meant the tail of a
            // burst was dropped whenever DataAvailable happened to be false at that instant, and it
            // put a whole poll interval between output being produced and being readable.
            _pump = Task.Run(() => PumpAsync(_pumpLifetime.Token));
        }

        internal DateTimeOffset LastActivityUtc { get; private set; }

        internal SshTerminalOutputRing Output { get; } = new();

        /// <summary>
        /// A session whose shell has failed stays alive with its ring intact until the desktop has
        /// read what it already produced. Disposing on the first throw -- which is what used to
        /// happen -- threw away buffered output along with the connection.
        /// </summary>
        internal bool IsFaulted { get; private set; }

        internal bool IsConnected => !_disposed && !IsFaulted && _client.IsConnected && _shell.CanRead;

        /// <summary>True once a faulted session has handed over everything it still held.</summary>
        internal bool IsDrained => IsFaulted && Output.BufferedBytes == 0;

        private async Task PumpAsync(CancellationToken cancellationToken)
        {
            var buffer = new byte[SshTerminalIpcContract.MaximumChunkBytes];
            try
            {
            ...
            catch (Exception error) when (error is IOException or SshConnectionException or
