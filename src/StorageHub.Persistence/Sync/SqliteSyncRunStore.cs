...
public sealed class SqliteSyncRunStore(SingleWriterSqliteDatabase database) : ISyncRunStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!await ProfileMatchesAsync(
                    ...
                    draft.ExpectedPolicySha256,
                    cancellationToken,
                    requireEnabled: false).ConfigureAwait(false) ||
    ...
    /// <summary>
    /// Binds a row to an exact profile revision and policy.
    ///
    /// <paramref name="requireEnabled"/> separates the two callers. Dispatching a run for
    /// execution must refuse a disabled profile. Recording a preview must not: a preview mutates
    /// nothing, and the editor lets an operator review a disabled profile by hand, so demanding
    /// enabled here rejected the write and surfaced as "the preview could not be persisted
    /// because its profile or trigger changed" when nothing had changed at all.
    /// </summary>
        ...
        bool requireEnabled = true)
    {
        await using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = requireEnabled
            ? """
            ...
            SELECT COUNT(*) FROM sync_profiles
            WHERE sync_profile_id = $id AND profile_revision = $revision
              AND lower(policy_hash) = lower($hash);
            """;
