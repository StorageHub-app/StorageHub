...
public sealed class SyncManagementIpcCommandService : IAgentIpcCommandHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // Distinct from UnavailableFailure(): that one means the sync service itself declined,
            // while this is an unexpected throw. Both used to report the same code, so a defect in
            // preview generation was indistinguishable from a busy or stopped service.
            return PreviewFailure(request.ProfileId, PreviewFaultedFailure());
        }
        finally
        {
    ...
    private static StorageIpcFailure PreviewFaultedFailure() => new(
