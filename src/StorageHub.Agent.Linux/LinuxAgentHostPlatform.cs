...
public sealed class LinuxAgentHostPlatform : IAgentHostPlatform
{
    ...
    /// <summary>
    /// None. Registering a systemd user unit needs no privilege, so there is nothing to hand to an
    /// elevated second process the way a service install must be.
    /// </summary>
    public Task<int?> TryHandleAdministrativeCommandAsync(IReadOnlyList<string> arguments) =>
