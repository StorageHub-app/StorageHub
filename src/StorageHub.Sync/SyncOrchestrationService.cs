...
public sealed class SyncOrchestrationService : ISyncOrchestrationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // "Disabled" means "never run this on a schedule". Reviewing one by hand is read-only, and
        // the editor promises exactly that ("Disabled profiles can still be reviewed manually"),
        // so a manual preview is allowed through. Blocking it made that promise false, and because
        // the refusal was reported as Unavailable the operator was told the service was down
        // rather than that the profile was simply switched off. Execution remains gated in
        // ApproveAndDispatchAsync, which still requires an enabled profile.
        if (!profile.Enabled && trigger == SyncPreviewTrigger.Scheduled)
        {
            return Fail<SyncPreviewResult>(
        ...
        // proof for a scheduled run: a profile switched off mid-scan must not keep a scheduled
        // preview alive, but it must not invalidate a manual review either, or disabling a profile
        // while looking at it would fail with a misleading "changed during scan".
        var currentProfile = await _profiles.GetAsync(profileId, cancellationToken).ConfigureAwait(false);
        if (currentProfile is null ||
            (trigger == SyncPreviewTrigger.Scheduled && !currentProfile.Enabled) ||
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // A disabled profile is still refused, but say so plainly. Folding it into
        // "profile_changed" sent the operator hunting for an edit that never happened, when the
        // plan was reviewable precisely because reviewing a disabled profile is allowed.
        if (profile is not null && !profile.Enabled &&
            profile.Revision == preview.ProfileRevision &&
            StringComparer.Ordinal.Equals(profile.PolicySha256, preview.ProfilePolicySha256))
        {
            return Fail<SyncPreviewRecord>(
                "sync.approval.profile_disabled",
                StorageFailureKind.Validation,
                "The sync profile is disabled, so its plan cannot be dispatched.");
        }
