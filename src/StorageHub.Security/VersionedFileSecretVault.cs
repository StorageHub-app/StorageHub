...
public sealed class VersionedFileSecretVault : ISecretVault, IDisposable
{
    ...
    /// <summary>
    /// Writes a secret under a reference the caller already holds, for moving a vault between
    /// protection schemes.
    ///
    /// <see cref="CreateAsync"/> always mints a new reference, which is right for new secrets and
    /// useless for a migration: connection profiles store the reference, so a migrated vault whose
    /// entries were renamed would leave every saved connection pointing at nothing. This refuses
    /// to replace an existing entry, so a partially completed migration can be re-run without
    /// silently overwriting whatever already arrived.
    /// </summary>
    /// <returns><see langword="false"/> when the reference already exists.</returns>
    public async ValueTask<bool> ImportAsync(
        SecretReference reference,
        ReadOnlyMemory<byte> secret,
        CancellationToken cancellationToken = default)
    {
