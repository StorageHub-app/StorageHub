...
public class LiveSyncTests
{
    ...
    public async Task AScheduleBuiltInTheManagerIsAcceptedByTheAgent()
    {
        ...
        try
        {
            ...
            Assert.False(
                saved.NextRun == Localization.Ui.Schedules.NoFutureRun ||
                    saved.NextRun == Localization.Ui.Schedules.NotScheduledUntilSaved,
            ...
            Assert.Equal(
                Localization.Ui.Sync.TaskDisabled,
    ...
    private static async Task RemoveConnectionsAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in listed.Where(
            static entry => entry.DisplayName is SourceName or DestinationName))
        {
            _ = await Controller()
