...
public sealed class ScheduleManagerFormTests
{
    ...
    public void Friendly_builder_creates_weekly_cron_and_keeps_the_real_time_zone_id()
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            ...
            var frequency = Find<StorageHubChoiceField>(form, "ScheduleFrequency");
            frequency.SelectedIndex = frequency.Items.Cast<object>()
                .Select((item, index) => (item, index))
                .Single(entry => entry.item.ToString() == "Every week")
                .index;
            Find<StorageHubChoiceField>(form, "ScheduleWeekDay").SelectedItem = DayOfWeek.Tuesday;
            Find<StorageHubTimeField>(form, "ScheduleTime").Value = DateTime.Today.AddHours(14).AddMinutes(30);

            var created = form.SaveCurrentScheduleAsync().GetAwaiter().GetResult();
            var timeZone = Find<StorageHubChoiceField>(form, "ScheduleTimeZone");
            ...
            Assert.False(Find<StorageHubTextField>(form, "ScheduleCron").Visible);
        });
    }
