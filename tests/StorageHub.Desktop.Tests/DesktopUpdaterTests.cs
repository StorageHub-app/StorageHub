...
public sealed class DesktopUpdaterTests
{
    ...
    public void PreferencesStoreRejectsRelativePaths()
    {
        _ = Assert.Throws<ArgumentException>(() =>
            new DesktopConfigStore("settings.json"));
    }
    ...
    public void AppearanceChoicesRoundTripWithoutChangingSchema(DesktopAppearance appearance)
    {
        ...
        Assert.Contains(
            $"\"schemaVersion\": {DesktopConfigSchema.Current}",
            File.ReadAllText(fixture.Store.GeneralPath),
    ...
    public void PinnedAndRecentWorkspacesSurviveRestart()
    {
        ...
        var restored = new DesktopConfigStore(fixture.SettingsDirectory).Load();
    ...
    public void AFullWorkspaceListStaysWellInsideTheSizeLoadWillAccept()
    {
        ...
        Assert.InRange(
            new FileInfo(fixture.Store.WorkspacesPath).Length, 1, DesktopConfigStore.MaximumFileBytes);
    ...
    private sealed class SettingsFixture : IDisposable
    {
        internal SettingsFixture()
        {
            ...
            SettingsDirectory = System.IO.Path.Combine(Directory, "Desktop");
            Path = System.IO.Path.Combine(SettingsDirectory, "settings.json");
            Store = new DesktopConfigStore(SettingsDirectory);
        }

        internal string Directory { get; }

        /// <summary>Where the store keeps its files, and where a legacy settings.json is put to migrate.</summary>
        internal string SettingsDirectory { get; }

        internal string Path { get; }

        internal DesktopConfigStore Store { get; }
