    ...
    public void TheGoMenuOffersAWayForwardWhenNothingIsFavorited()
    {
        WithShell(
            ...
            (main, directory) =>
            {
                ...
                // The empty state explains itself and stops there. It used to repeat a command
                // that already sits in the Connections menu, and favourites are marked in the
                // connections panel now, so the hint points there instead.
                var empty = Single(items, "No favorite connections");
                Assert.False(empty.Enabled);
                Assert.Contains("connections panel", empty.ToolTipText, StringComparison.Ordinal);
                Assert.DoesNotContain(items, item => item.Text == "New Connection...");
            });
    }
