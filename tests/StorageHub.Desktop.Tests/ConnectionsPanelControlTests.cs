...
public sealed class ConnectionsPanelControlTests
{
    ...
    public void ThePanelGroupsByFavouriteTypeAndFolderAndNeverInventsProfiles()
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            ...
            // Six rows for five profiles: the enabled favourite is listed under Favourites and
            // again in its own folder.
            ...
            Assert.Equal("Healthy · 42 ms", cards.First(static card => card.Name == "Favorite").State);
            Assert.Equal(
                "FTP saved profile",
    ...
    public void SelectingAConnectionFillsTheDetailSectionWithoutOpeningIt()
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            ...
            var values = Descendants<DetailFactRow>(panel)
                .SelectMany(static row => new[] { row.Key, row.Value })
                .Concat(Descendants<Label>(panel).Select(static label => label.Text))
                .ToArray();
    ...
    public void TheDetailSectionGroupsTheProfileIntoKeyValueSections()
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            ...
            // The grid is drawn rather than composed from labels, so the rows are read from the
            // row controls themselves: keys and values, not whatever widget happens to hold them.
            Assert.Equal(
                ["Server", "Authentication", "Security", "Transfer", "Organisation", "Status"],
                Descendants<DetailCategoryHeader>(panel).Select(static header => header.Text));
            var labels = Descendants<DetailFactRow>(panel)
                .SelectMany(static row => new[] { row.Key, row.Value })
                .Concat(Descendants<Label>(panel).Select(static label => label.Text))
                .ToArray();
    ...
    public void EmptyProfileFieldsAreLeftOutRatherThanDrawnBlank()
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            ...
            var keys = Descendants<DetailFactRow>(panel)
                .SelectMany(static row => new[] { row.Key, row.Value })
                .ToArray();
    ...
    private static StorageHubTextField SearchBox(Control root) =>
        Assert.Single(
            Descendants<StorageHubTextField>(root),
