...
public sealed class BrowserPaneControlTests
{
    ...
    public void ReplacingVirtualRowsPrimesIconsAndImmediatelyReturnsEveryNewRow()
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            var client = new FakeRemoteStorageClient(Guid.NewGuid());
            using var pane = new BrowserPaneControl("Pane 1", showLocalDefault: true, new RemoteBrowserController(client));
