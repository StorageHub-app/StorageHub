...
public sealed class ThemeTests
{
    ...
    public void Disabled_primary_buttons_have_an_unambiguous_disabled_surface(DesktopAppearance appearance)
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            ...
            // The button paints its own rounded surface, so the colours live in what it draws
            // rather than in BackColor -- which stays transparent so the corners can show what is
            // behind them.
            Assert.Equal(StorageHubTheme.CurrentPalette.SurfaceMuted, button.ResolveSurface().Fill);
            Assert.Equal(StorageHubTheme.CurrentPalette.DisabledText, button.ResolveSurface().Text);

            button.Enabled = true;
            Assert.Equal(StorageHubTheme.CurrentPalette.Primary, button.ResolveSurface().Fill);
            Assert.Equal(
                StorageHubTheme.ContrastText(StorageHubTheme.CurrentPalette.Primary),
                button.ResolveSurface().Text);
    ...
    // Heights around the point where the rows just fit. A column fill that adds or removes a
    // scrollbar changes the client size, which raises Resize, which fills again: the fill used
    // to nest inside itself until the native control gave up, and the overview page sat at full
    // CPU for half a minute on every such layout.
    [Theory]
    [InlineData(48)]
    [InlineData(64)]
    [InlineData(80)]
    [InlineData(96)]
    [InlineData(112)]
    public void The_trailing_column_fill_does_not_feed_back_through_resize(int height)
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            using var list = new ListView { View = View.Details, Width = 400, Height = height };
            using var host = new Form();
            host.Controls.Add(list);
            host.CreateControl();
            list.Columns.Add("A", 150);
            ...
            {
                list.Items.Add(new ListViewItem([$"row {index}", "b", "c"]));
            }

            var resizes = 0;
            list.Resize += (_, _) => resizes++;
            ...
            Assert.True(resizes < 12, $"Two width changes raised Resize {resizes} times.");
            var expected = Math.Max(80, list.ClientSize.Width - 300 - 2);
            Assert.Equal(expected, list.Columns[^1].Width);
        });
    }

    [Fact]
    public void Refitting_an_unchanged_trailing_column_leaves_the_columns_alone()
    {
        SyncRunReviewControlTests.RunOnSta(() =>
        {
            using var list = new ListView { View = View.Details, Width = 700, Height = 120 };
            using var host = new Form();
            host.Controls.Add(list);
            host.CreateControl();
            list.Columns.Add("A", 150);
            ...
            var changes = 0;
            list.ColumnWidthChanged += (_, _) => changes++;

            // Nothing about the list has changed, so nothing should be written: every column
            // assignment repaints the header and the rows, which is a flash on each step of a
            // window drag when it happens on every Resize.
            StorageHubTheme.FitTrailingColumn(list);
