...
public sealed class SqliteConnectionProfileRepositoryTests : IDisposable
{
    ...
    public async Task Every_supported_provider_endpoint_and_authentication_round_trips()
    {
        ...
        var profiles = new[]
        {
            ...
            ConnectionProfile.Create(ConnectionProfileId.New(), new ConnectionProfileMetadata("FTP legacy server"),
                new FtpEndpoint("old-ftp.example.test", 21, true, serverOptions: new FtpServerOptions(
                    "Europe/Copenhagen", FtpListingFormat.Windows, FtpDataConnectionMode.Active,
                    50_000, 50_100, System.Net.IPAddress.Parse("203.0.113.7"))),
                new UsernamePasswordAuthentication("ftp-user", password), OptionsForConnection(), now),
            ConnectionProfile.Create(ConnectionProfileId.New(), new ConnectionProfileMetadata("FTPS provider"),
                new FtpsEndpoint("ftps.example.test", 990, FtpsTlsMode.Implicit,
                    ...
                    serverOptions: new FtpServerOptions(listingFormat: FtpListingFormat.Unix)),
