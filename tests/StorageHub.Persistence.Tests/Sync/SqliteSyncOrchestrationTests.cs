...
public sealed class SqliteSyncOrchestrationTests : IAsyncLifetime, IDisposable
{
    ...
    /// <summary>
    /// Disabling a profile means "never run this on a schedule", not "hide it". The editor offers
    /// exactly that ("Disabled profiles can still be reviewed manually"), but the preview used to
    /// refuse outright -- and reported the refusal as Unavailable, so the dialog told the operator
    /// the sync service was down when the profile was merely switched off.
    /// </summary>
    [Fact]
    public async Task A_disabled_profile_can_still_be_reviewed_by_hand()
    {
        await SetProfileEnabledAsync(false);
        var left = new ReadOnlyEndpointSession(
            _leftId,
            "left-root-identity",
            [File(_leftId, "left-root-identity", "hello.txt", 5, "left-v1", "left-etag")]);
        ...
        var preview = await service.GeneratePreviewAsync(
            ...
            "manual:disabled-review");

        Assert.True(preview.IsSuccess, preview.Error?.Message);
        Assert.Single(preview.Value.Plan.Operations);
        Assert.Equal(0, left.MutationCalls + right.MutationCalls);
    }

    /// <summary>
    /// The half that must stay shut: a disabled profile is refused for scheduled work, and as a
    /// validation failure rather than a transient one, so no retry policy parks it forever.
    /// </summary>
    [Fact]
    public async Task A_disabled_profile_refuses_scheduled_work_without_looking_transient()
    {
        await SetProfileEnabledAsync(false);
        var service = CreateService(new TestConnector(
            new ReadOnlyEndpointSession(_leftId, "left-root-identity", []),
            new ReadOnlyEndpointSession(_rightId, "right-root-identity", [])));

        var scheduled = await service.GeneratePreviewAsync(
            _profileId,
            SyncPreviewTrigger.Scheduled,
            "outbox:disabled-schedule");

        Assert.True(scheduled.IsFailure);
        Assert.Equal("sync.profile.disabled", scheduled.Error.Code);
        Assert.Equal(StorageFailureKind.Validation, scheduled.Error.Kind);
        Assert.False(scheduled.Error.IsTransient);
    }

    /// <summary>A reviewed disabled profile still may not execute; approval is the real gate.</summary>
    [Fact]
    public async Task A_disabled_profile_still_cannot_be_approved_for_execution()
    {
        var left = new ReadOnlyEndpointSession(
            _leftId,
            "left-root-identity",
        ...
        var preview = await service.GeneratePreviewAsync(
            _profileId,
            SyncPreviewTrigger.Manual,
            "manual:disable-after-preview");
        Assert.True(preview.IsSuccess, preview.Error?.Message);
        await SetProfileEnabledAsync(false);

        var approved = await service.ApproveAndDispatchAsync(
            preview.Value.Preview.SyncRunId,
            preview.Value.Preview.State.Revision,
            preview.Value.Preview.ApprovalChallengeSha256);

        Assert.True(approved.IsFailure);
        Assert.Equal("sync.approval.profile_changed", approved.Error.Code);
        Assert.Equal(0, left.MutationCalls + right.MutationCalls);
    }

    private async Task SetProfileEnabledAsync(bool enabled)
    {
        var desired = new SyncProfile(
            _profile.ProfileId,
            _profile.DisplayName,
            ...
            _profile.CreatedAtUtc,
            _profile.UpdatedAtUtc);
        var updated = await new SqliteSyncProfileRepository(_database, new FixedTimeProvider(Now))
            .UpdateAsync(desired, expectedRevision: _profile.Revision);
        Assert.Equal(SyncProfileWriteStatus.Succeeded, updated.Status);
