...
public sealed class DatabaseFoundationTests : IDisposable
{
    ...
    public async Task Credential_table_can_store_only_opaque_metadata_not_secret_values()
    {
        ...
        // The key store holds opaque vault references and derived summaries only.
        Assert.Contains("material_reference", columns);
        Assert.Contains("passphrase_reference", columns);
        Assert.Contains("summary_json", columns);
