...
public sealed class SshTerminalIntegrationTests : IDisposable
{
        ...
        string marker)
    {
        ...
        {
            // The acknowledgement is what moves the cursor on. Reading without it would replay the
            // same bytes for ever, which is exactly the property the ring is there to provide.
            ...
            consumed = read.StartSequence + read.Content.Length;
        }
